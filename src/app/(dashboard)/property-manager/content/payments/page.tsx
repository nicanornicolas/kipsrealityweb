...
export default function PaymentsPage() {
  ...
  const [viewingReceipt, setViewingReceipt] = useState<FullReceipt | null>(
    null,
  );
  ...
  const getTenantInfo = (payment: Payment) => {
    ...
    const fullName =
      `${firstName} ${lastName}`.trim() || tenant.email || 'Unknown Tenant';
  ...
  useEffect(() => {
    ...
    if (modalPropertyFilter) {
      filtered = filtered.filter(
        (inv) => inv.Lease.property.id === modalPropertyFilter,
      );
    }

    if (modalUnitFilter) {
      filtered = filtered.filter(
        (inv) => inv.Lease.unit.unitNumber === modalUnitFilter,
      );
    }
  ...
  async function recordPayment() {
    ...
    const invoice =
      filteredInvoices.find((inv) => inv.id === selectedInvoice) ||
    ...
    if (
      ...
      return toast.error(
        `Credit card payments must be for the full remaining balance of USD ${remaining.toFixed(2)}`,
      );
    }

    if (paymentMethod === 'CASH' && amount > remaining) {
      return toast.error(
        `Payment amount cannot exceed remaining balance of USD ${remaining.toFixed(2)}`,
      );
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (isPaidInFull) {
          ...
          toast.success(
            `Partial payment of USD ${amount.toFixed(2)} recorded. Remaining: USD ${data.remaining.toFixed(2)}`,
          );
        }
                ...
                <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setModalPropertyFilter(e.target.value)
                          }
                  ...
                  <div>
                    ...
                    >
                      ...
                      {filteredInvoices.map((inv) => {
                        const validPayments =
                          inv.payment?.filter((p) => !p.isReversed) || [];
                        ...
                        return (
                          <option key={inv.id} value={inv.id}>
                            #{inv.id.slice(0, 8)} - {inv.type} -{' '}
                            {inv.Lease.property.city} Unit{' '}
                            {inv.Lease.unit.unitNumber} - Tenant: {tenantName} -
                            USD {remaining.toFixed(2)} remaining
                          </option>
                        );
                      })}
                    </select>
                    {filteredInvoices.length === 0 && (
                      <p className="text-xs text-amber-600 mt-1">
                        No pending invoices found with current filters. Try
                    ...
                    <div className="bg-slate-50 rounded-lg p-4 space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-semibold">
                          USD {selectedInvoiceData.amount.toFixed(2)}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-600">Already Paid:</span>
                        <span className="font-semibold text-emerald-600">
                          USD {paidSoFar.toFixed(2)}
                        </span>
                      </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        setPaymentMethod(
                          e.target.value as 'CASH' | 'CREDIT_CARD',
                        );
                        if (
                  ...
                  <div>
                    ...
                    {paymentMethod === 'CASH' && remainingBalance > 0 && (
                      ...
                      >
                        Use full remaining balance (USD{' '}
                      ...
                      >
                        {paymentMethod === 'CASH' ? (
                          <>
                            ...
                            <p>
                              You can record partial payments. The invoice will
                              remain open until fully paid.
                            </p>
                          </>
                        ) : (
                          <>
                            ...
                            <p>
                              Credit card payments must be for the full
                              remaining balance.
                            </p>
                          </>
                        )}
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
                <p className="text-sm text-red-800">
                  ...
                  <span className="font-bold">
                    #{paymentToReverse.id.slice(0, 8)}
                  </span>{' '}
                  of{' '}
                  <span className="font-bold">
                    $ {paymentToReverse.amount.toFixed(2)}
                  </span>
                  ?
                </p>
              </div>
              ...
              <div>
                ...
                <div className="text-2xl font-bold text-white">
                  ...
                  {validPayments.length > 0
                    ? (totalAmount / validPayments.length).toFixed(2)
                    : '0.00'}
                </div>{' '}
              </div>
            </div>
                  ...
                  {payments.map((p) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <div className="text-xs text-slate-500">
                          {new Date(p.paidOn).toLocaleTimeString()}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-slate-900">
                          {p.invoice.Lease?.property?.city ?? 'N/A'}
                        </div>
                        <div className="text-xs text-slate-500">
                          Unit {p.invoice.Lease?.unit?.unitNumber ?? 'N/A'}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
