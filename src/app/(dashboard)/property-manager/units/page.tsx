...
export default function ManageUnitsPage() {
  ...
  const handleListingDecision = async (
    ...
    try {
      ...
      const refreshResponse = await fetch(
        `/api/units?${queryParam}&include=listing,lease`,
      );
  ...
  const canCreateListing = (unit: Unit) => {
    ...
    return (
      unit.id && !unit.listing && (!unit.leases || unit.leases.length === 0)
    );
  };
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Configure unit details and marketplace listings
        </p>
      </div>
            ...
            <CardContent className="p-6 space-y-4">
              ...
              {unit.id ? (
                <div className="text-gray-700 space-y-2">
                  ...
                  <p>
                    <strong>Rent:</strong> $
                    {unit.rentAmount?.toLocaleString() ?? 'N/A'}
                  </p>
                  {unit.listing && (
                    <p>
                      <strong>Listed Price:</strong> $
                      {unit.listing.price?.toLocaleString()}
                    </p>
                  )}
                  {unit.leases && unit.leases.length > 0 && (
                    <p className="text-blue-600">
                      <strong>Lease Status:</strong>{' '}
                      {unit.leases[0].leaseStatus}
                    </p>
                  )}
                </div>
              ) : (
