...
export default function EnhancedTenantDashboard() {
  ...
  const filteredLeases = leases.filter((lease) => {
    ...
    const matchesStatus =
      statusFilter === 'ALL' || lease.leaseStatus === statusFilter;
  ...
  const stats = leases.reduce(
    (acc, lease) => {
      ...
      const totalPaid =
        lease.invoice?.reduce((sum: number, inv: Invoice) => {
          const paid =
            inv.payment
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-slate-600 mt-1">
                Manage tenants, applications, invitations & payments
              </p>
            </div>
            <button
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Paid: $ {(stats.totalPaid / 1000).toFixed(0)}k
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-amber-600">
                  {stats.pendingApplications + stats.pendingInvites}
                </p>
                  ...
                  {filteredLeases.length === 0 ? (
                    ...
                    filteredLeases.map((lease) => (
                      ...
                      >
                              ...
                              return (
                                <>
                                  ...
                                  <div className="flex justify-between">
                                    ...
                                    <span
                                      className={`font-semibold ${balance > 0 ? 'text-red-600' : 'text-green-500'}`}
                                    >
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold border ${getStatusColor(lease.leaseStatus)}`}
                          >
                            {lease.leaseStatus}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent
                              align="end"
                              className="w-48 bg-white shadow-lg rounded-md border border-slate-200"
                            >
                              <DropdownMenuItem
                                onClick={() =>
                                  (window.location.href = `/property-manager/content/lease/${lease.id}`)
                                }
                              ...
                              <DropdownMenuItem
                                onClick={() =>
                                  lease.tenant && openEditModal(lease.tenant)
                                }
                                className="flex items-center gap-2"
                              >
                  ...
                  {filteredApplications.length === 0 ? (
                    ...
                    filteredApplications.map((app) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <p className="font-medium text-slate-900">
                            {app.property?.name || app.property?.city}
                          </p>
                          <p className="text-sm text-slate-500">
                        ...
                        <td className="px-6 py-4">
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-slate-600">
                              $ {(app.monthlyIncome || 0).toLocaleString()}
                            </p>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold border ${getStatusColor(app.status)}`}
                          >
                            {app.status}
                          </span>
                        </td>
                  ...
                  {filteredInvites.length === 0 ? (
                    ...
                    filteredInvites.map((invite) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold border ${invite.accepted ? getStatusColor('APPROVED') : getStatusColor('PENDING')}`}
                          >
                            {invite.accepted ? 'Accepted' : 'Pending'}
                          </span>
                        </td>
                          ...
                          <div className="flex gap-2">
                            {!invite.accepted && (
                              <>
                                ...
                                >
                                  <RefreshCw
                                    className={`w-4 h-4 ${resendingInviteId === invite.id ? 'animate-spin' : ''}`}
                                  />
                                  {resendingInviteId === invite.id
                                    ? 'Sending...'
                                    : 'Resend'}
            ...
            <div className="bg-white rounded-lg shadow-lg rounded-2xl max-w-md w-full">
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  >
                    <option value="">
                      -- Select a lease ({leases.length} available) --
                    </option>
                    {leases.map((lease) => {
                  ...
                  {leases.length === 0 && (
                    <p className="text-amber-600 text-xs mt-2">
                      No leases available. Please create a lease first.
                    </p>
                  )}
                </div>
            ...
            <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="sticky top-0 bg-white border-b border-slate-200 p-6 flex justify-between items-start">
                <div>
                  ...
                  <p className="text-slate-600 mt-1">
                    Complete review of tenant application
                  </p>
                </div>
                <button
              ...
              <div className="p-6 space-y-6">
                ...
                >
                  ...
                  <span
                    className={`inline-flex px-4 py-2 rounded-lg text-sm font-semibold border ${getStatusColor(selectedApplication.status)}`}
                  >
                    {selectedApplication.status}
                  </span>
                </div>
                    ...
                    <div className="bg-slate-50 p-4 rounded-lg">
                      ...
                      <p className="text-slate-900 font-semibold mt-1">
                        {selectedApplication.dob
                          ? formatDate(selectedApplication.dob)
                          : 'N/A'}
                      </p>
                    </div>
                    <div className="bg-slate-50 p-4 rounded-lg">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-50 p-4 rounded-lg">
                      ...
                      <p className="text-slate-900 font-semibold mt-1">
                        {selectedApplication.property?.name ||
                    ...
                    <div className="bg-slate-50 p-4 rounded-lg">
                      ...
                      <p className="text-slate-900 font-semibold mt-1">
                        {selectedApplication.unit?.unitNumber || 'N/A'}
                      </p>
                    </div>
                    <div className="bg-slate-50 p-4 rounded-lg">
                      <p className="text-sm text-slate-600 font-medium">
                      ...
                      <p className="text-slate-900 font-semibold mt-1">
                        ${' '}
                        {(
                          selectedApplication.unit?.rentAmount || 0
                        ).toLocaleString()}
                      </p>
                    </div>
                    <div className="bg-slate-50 p-4 rounded-lg">
                      ...
                      <p className="text-slate-900 font-semibold mt-1">
                        {formatDate(selectedApplication.moveInDate)}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-50 p-4 rounded-lg">
                      ...
                      <p className="text-slate-900 font-semibold mt-1">
                        {selectedApplication.employerName || 'N/A'}
                      </p>
                    </div>
                    <div className="bg-slate-50 p-4 rounded-lg">
                      <p className="text-sm text-slate-600 font-medium">
                      ...
                      <p className="text-slate-900 font-semibold mt-1">
                        {selectedApplication.jobTitle || 'N/A'}
                      </p>
                    </div>
                    <div className="col-span-2 bg-green-50 border border-navy-200 p-4 rounded-lg">
                      ...
                      <p className="text-green-700 font-bold text-xl mt-1">
                        ${' '}
                        {(
                          selectedApplication.monthlyIncome || 0
                        ).toLocaleString()}
                      </p>
                      <p className="text-xs text-slate-600 mt-2">
                        Rent-to-Income Ratio:{' '}
                        {selectedApplication.unit?.rentAmount &&
                ...
                <div className="p-6 border-t border-gray-200 bg-gray-50 flex gap-3 justify-end">
                  ...
                  {(selectedApplication.status === 'Approved' ||
                    ...
                    <button
                      onClick={() =>
                        proceedToLease(
                          selectedApplication as unknown as TenantApplication,
                        )
                      }
