...
import { postProperty, PropertyPayload } from '@rentflow/property/client';
...
export default function PropertyForm({ onSuccess }: PropertyFormProps) {
  ...
  const selectedPropertyTypeName = propertyTypes
    .find((type) => type.id === selectedPropertyTypeId)
    ?.name?.toLowerCase();

  const [country, city, zipCode, address] = watch([
    'country',
    'city',
    'zipCode',
    'address',
  ...
  const onSubmit = async (data: Property) => {
    ...
    if (!mapCoordinates) {
      toast.error('Location Missing', {
        description:
          'Please pin the property location on the map before submitting.',
      });
      return;
    }
    ...
    try {
      ...
      if (data.images && data.images.length > 0) {
        ...
        setUploadProgress(
          `Uploading ${data.images.length} images to Cloudinary...`,
        );
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="bg-gray-50 rounded-2xl p-6 border border-gray-100">
              ...
              <p className="text-xs text-gray-500 mt-2">
                Upload multiple images (JPG, PNG). Images will be optimized
                automatically.
              </p>
            </div>
