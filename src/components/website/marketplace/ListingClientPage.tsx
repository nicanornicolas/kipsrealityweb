...
export function MarketplaceClientPage({
  ...
  const handleApply = async (listing: MarketplaceItem) => {
    ...
    const propertyId =
      listing.propertyId ?? listing.unit?.property?.id ?? listing.property?.id;

    if (!unitId || !propertyId) {
      toast.error(
        "This listing doesn't have an associated unit or property. Please contact support.",
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/units/by-id/${unitId}/application-eligibility`,
      );
      if (!response.ok) {
        toast.error(
          'Unable to check unit availability. Please try again later.',
        );
        return;
      }
      ...
      if (!eligibility.isEligible) {
        toast.error(
          eligibility.reason ||
            'This unit is not currently available for applications.',
        );
        return;
      }
      ...
      setSelectedListing({
        ...
        unit: listing.unit || {
          id: unitId,
          property: { id: propertyId, name: listing.property?.name || '' },
        },
      ...
      toast.error('Unable to check unit availability. Please try again later.');
    }
  };
