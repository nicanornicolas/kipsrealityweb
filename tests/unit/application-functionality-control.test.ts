...
vi.mock('@prisma/client', () => {
  ...
  const mockInstance = {
    ...
    $transaction: vi.fn().mockImplementation(async (callback) => callback(mockInstance)),
    user: {
      create: vi.fn(),
      deleteMany: vi.fn()
    },
...
describe('Property 13: Application Functionality Control', () => {
  ...
  describe('Unit with active listing', () => {
    it('Property 13.1: Units with active listings should allow applications', async () => {
      ...
      const eligibility = await applicationControlService.checkApplicationEligibility(unitId);
  ...
  describe('Consistency checks', () => {
    it('Property 13.4: Application eligibility should be consistent across multiple checks', async () => {
      ...
      const check1 = await applicationControlService.checkApplicationEligibility(unitId);
      const check2 = await applicationControlService.checkApplicationEligibility(unitId);
      const check3 = await applicationControlService.checkApplicationEligibility(unitId);
