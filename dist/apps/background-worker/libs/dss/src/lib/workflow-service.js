...
var workflow_service_exports = {};
__export(workflow_service_exports, {
  WorkflowService: () => WorkflowService
});
module.exports = __toCommonJS(workflow_service_exports);
...
class WorkflowService {
  ...
  async sendDocument(documentId, organizationId) {
    ...
    const updatedDoc = await import_iam.prisma.dssDocument.update({
      where: { id: documentId },
      data: {
    ...
    for (const signer of firstSigners) {
      await import_utilities.emailQueue.add("send-signature-invite", {
        email: signer.email,
  ...
  async canParticipantSign(documentId, participantId) {
    ...
    if (!document || document.status !== import_client.DssDocumentStatus.SENT && document.status !== import_client.DssDocumentStatus.IN_SIGNING) {
      return false;
    }
    ...
    const currentActiveStep = document.participants.filter((p) => !p.hasSigned).reduce(
  ...
  async signDocument(payload) {
    ...
    if (result.isCompleted) {
      await import_utilities.dssPdfQueue.add("generate-signed-pdf", {
        documentId: payload.documentId,
      ...
      const nextStepOrder = result.document.participants.filter((p) => !p.hasSigned).reduce((min, p) => p.stepOrder < min ? p.stepOrder : min, Infinity);
