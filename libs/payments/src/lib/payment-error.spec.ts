...
} from '@/lib/payment/payment-error-handler';
...
describe('Payment Error Handler', () => {
  ...
  describe('PaymentError Class', () => {
    ...
    describe('Static factory methods', () => {
      it('should create network error', () => {
        const error = PaymentError.createNetworkError(
          'Connection failed',
      ...
      it('should create authentication error', () => {
        const error = PaymentError.createAuthenticationError(
          'Invalid credentials',
      ...
      it('should create validation error', () => {
        const error = PaymentError.createValidationError(
      ...
      it('should create fraud error', () => {
        const error = PaymentError.createFraudError('Suspicious activity', {
          score: 85,
        });
      ...
      it('should create insufficient funds error', () => {
        const error =
          PaymentError.createInsufficientFundsError('Balance too low');
  ...
  describe('Error Normalization', () => {
    ...
    it('should normalize payment gateway errors', async () => {
      const gatewayError = new Error(
        'M-Pesa payment gateway error: Connection timeout',
      );
    ...
    it('should preserve existing PaymentError instances', async () => {
      const existingError = PaymentError.createFraudError(
        'Suspicious pattern',
  ...
  describe('Retry Logic', () => {
    ...
    it('should respect max attempts', async () => {
      ...
      await expect(
        errorHandler.withRetry(operation, 'test-max-attempts'),
      ).rejects.toThrow();
    ...
    it('should not retry non-retryable errors', async () => {
      ...
      await expect(
        errorHandler.withRetry(operation, 'test-non-retryable'),
      ).rejects.toThrow();
  ...
  describe('Error Classification', () => {
    ...
    it('should classify fraud detection errors', () => {
      const error = PaymentError.createFraudError('High risk score detected', {
        score: 90,
      });
      expect(error.type).toBe(PaymentErrorType.FRAUD_DETECTED);
      expect(error.retryable).toBe(false);
