    ...
    fc.assert(
      fc.property(
        ...
        (unit, partialData) => {
          ...
          const partialWithoutDescription = {
            ...partialData,
            description: undefined,
          };

          const result = populateListingDefaults(
            unit,
            partialWithoutDescription,
          );
    ...
    fc.assert(
      fc.property(
        ...
        (unit, partialData) => {
          ...
          const now = new Date();
          const daysBefore =
            (now.getTime() - result.availabilityDate.getTime()) /
            (1000 * 60 * 60 * 24);
