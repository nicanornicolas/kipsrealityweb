      ...
      fc.asyncProperty(
        ...
        async (
            ...
            if (op.action === BulkListingActionType.LIST) {
              ...
              mockRemoveListing.mockResolvedValueOnce(
                shouldSucceed
                  ...
                  : {
                      success: false,
                      error: 'LISTING_NOT_FOUND' as any,
                      message: 'Mock listing not found',
                    },
              );
            }
          });
          ...
          if (result.success) {
            ...
            expect(bulkResult.summary.succeeded).toBe(
              bulkResult.successful.length,
            );
            expect(bulkResult.summary.failed).toBe(bulkResult.failed.length);
            expect(
              bulkResult.summary.succeeded + bulkResult.summary.failed,
            ).toBe(bulkResult.summary.total);
            ...
            const intersection = new Set(
              [...successfulSet].filter((x) => failedSet.has(x)),
            );
            expect(intersection.size).toBe(0);

            // 6. Verify individual operations were called correctly
            const listOperations = operations.filter(
              (op) => op.action === BulkListingActionType.LIST,
            );
            const unlinkOperations = operations.filter(
              (op) =>
                op.action === BulkListingActionType.UNLIST ||
                op.action === BulkListingActionType.SUSPEND,
            );

            expect(mockCreateListing).toHaveBeenCalledTimes(
              listOperations.length,
            );
            expect(mockRemoveListing).toHaveBeenCalledTimes(
              unlinkOperations.length,
            );
      ...
      fc.asyncProperty(
        ...
        async (
          ...
          const mixedOperations = [
            ...operations
              .slice(0, Math.ceil(operations.length / 3))
              .map((op) => ({
                ...op,
            ...
            ...operations
              .slice(
                Math.ceil(operations.length / 3),
                Math.ceil((2 * operations.length) / 3),
              )
              .map((op) => ({
                ...op,
            ...
            ...operations
              .slice(Math.ceil((2 * operations.length) / 3))
              .map((op) => ({
                ...op,
          ...
          if (result.success) {
            ...
            expect(new Set(allProcessedUnits).size).toBe(
              allProcessedUnits.length,
            );
