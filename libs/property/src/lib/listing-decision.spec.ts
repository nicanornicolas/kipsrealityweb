...
describe('Listing Decision Integration Property Tests', () => {
  it('Property 1: Listing Decision Creates Appropriate State - List Decision', async () => {
    await fc.assert(
      fc.property(mockUnitGenerator, (unit) => {
        ...
        const expectedListingData = {
          ...
          description:
            unit.listing?.description || `${unit.bedrooms || 0} bedroom unit`,
  ...
  it('Property 1: Listing Decision Creates Appropriate State - Private Decision', async () => {
    await fc.assert(
      fc.property(mockUnitGenerator, (unit) => {
  ...
  it('Property 1: Listing Decision Creates Appropriate State - State Consistency', async () => {
    await fc.assert(
      fc.property(mockUnitGenerator, decisionGenerator, (unit, decision) => {
        // Property: Decision outcome should be consistent with input

        // Simulate decision processing
        const resultingStatus =
          decision === 'list' ? ListingStatus.ACTIVE : ListingStatus.PRIVATE;
  ...
  it.skip('Property 1: Listing Decision Creates Appropriate State - Immediate Update', async () => {
    await fc.assert(
      fc.property(mockUnitGenerator, decisionGenerator, (unit, decision) => {
        // Property: Status should be updated immediately after decision

        const beforeDecisionStatus =
          unit.listing?.status || ListingStatus.PRIVATE;
        const afterDecisionStatus =
          decision === 'list' ? ListingStatus.ACTIVE : ListingStatus.PRIVATE;
  ...
  it('Property 1: Listing Decision Creates Appropriate State - Error Handling', async () => {
    await fc.assert(
      fc.property(mockUnitGenerator, decisionGenerator, (unit, decision) => {
        ...
        const hasValidUnitNumber =
          unit.unitNumber && unit.unitNumber.length > 0;
        const hasValidPropertyId =
          unit.propertyId && unit.propertyId.length > 0;
  ...
  it('Property 1: Listing Decision Creates Appropriate State - Marketplace Synchronization', async () => {
    await fc.assert(
      fc.property(mockUnitGenerator, (unit) => {
        ...
        Object.entries(statusVisibilityMap).forEach(
          ([status, expectedVisibility]) => {
  ...
  it('Property 1: Listing Decision Creates Appropriate State - Data Preservation', async () => {
    await fc.assert(
      fc.property(mockUnitGenerator, decisionGenerator, (unit, decision) => {
