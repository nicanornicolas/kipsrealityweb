...
describe('Listing Status Transitions Property Tests', () => {
  it('Property 8: Valid State Transitions - All defined transitions should be valid', async () => {
    await fc.assert(
      fc.property(
        ...
        fc.constantFrom(
          ...Object.entries(VALID_STATUS_TRANSITIONS).flatMap(
            ([from, toStates]) =>
  ...
  it('Property 8: Valid State Transitions - Invalid transitions should be rejected', async () => {
    await fc.assert(
      fc.property(
        ...
        fc.constantFrom(...Object.values(ListingStatus)),
        (fromStatus, toStatus) => {
          const isDefinedAsValid =
            VALID_STATUS_TRANSITIONS[fromStatus]?.includes(toStatus) ?? false;
    ...
    await fc.assert(
      fc.property(
        ...
        (status) => {
          ...
          const canTransitionToSelf =
            VALID_STATUS_TRANSITIONS[status].includes(status);
  ...
  it('Property 8: Valid State Transitions - Specific business rules', () => {
    ...
    expect(
      isValidStatusTransition(ListingStatus.PRIVATE, ListingStatus.ACTIVE),
    ).toBe(true);
    expect(
      isValidStatusTransition(ListingStatus.PRIVATE, ListingStatus.PENDING),
    ).toBe(true);
    expect(
      isValidStatusTransition(ListingStatus.PRIVATE, ListingStatus.SUSPENDED),
    ).toBe(false);
    expect(
      isValidStatusTransition(ListingStatus.PRIVATE, ListingStatus.EXPIRED),
    ).toBe(false);

    // ACTIVE can transition to PRIVATE, SUSPENDED, and EXPIRED
    expect(
      isValidStatusTransition(ListingStatus.ACTIVE, ListingStatus.PRIVATE),
    ).toBe(true);
    expect(
      isValidStatusTransition(ListingStatus.ACTIVE, ListingStatus.SUSPENDED),
    ).toBe(true);
    expect(
      isValidStatusTransition(ListingStatus.ACTIVE, ListingStatus.EXPIRED),
    ).toBe(true);
    expect(
      isValidStatusTransition(ListingStatus.ACTIVE, ListingStatus.PENDING),
    ).toBe(false);

    // SUSPENDED can transition to ACTIVE and PRIVATE
    expect(
      isValidStatusTransition(ListingStatus.SUSPENDED, ListingStatus.ACTIVE),
    ).toBe(true);
    expect(
      isValidStatusTransition(ListingStatus.SUSPENDED, ListingStatus.PRIVATE),
    ).toBe(true);
    expect(
      isValidStatusTransition(ListingStatus.SUSPENDED, ListingStatus.PENDING),
    ).toBe(false);
    expect(
      isValidStatusTransition(ListingStatus.SUSPENDED, ListingStatus.EXPIRED),
    ).toBe(false);

    // EXPIRED can transition to ACTIVE and PRIVATE
    expect(
      isValidStatusTransition(ListingStatus.EXPIRED, ListingStatus.ACTIVE),
    ).toBe(true);
    expect(
      isValidStatusTransition(ListingStatus.EXPIRED, ListingStatus.PRIVATE),
    ).toBe(true);
    expect(
      isValidStatusTransition(ListingStatus.EXPIRED, ListingStatus.PENDING),
    ).toBe(false);
    expect(
      isValidStatusTransition(ListingStatus.EXPIRED, ListingStatus.SUSPENDED),
    ).toBe(false);

    // PENDING can transition to ACTIVE and PRIVATE
    expect(
      isValidStatusTransition(ListingStatus.PENDING, ListingStatus.ACTIVE),
    ).toBe(true);
    expect(
      isValidStatusTransition(ListingStatus.PENDING, ListingStatus.PRIVATE),
    ).toBe(true);
    expect(
      isValidStatusTransition(ListingStatus.PENDING, ListingStatus.SUSPENDED),
    ).toBe(false);
    expect(
      isValidStatusTransition(ListingStatus.PENDING, ListingStatus.EXPIRED),
    ).toBe(false);
  });
});
