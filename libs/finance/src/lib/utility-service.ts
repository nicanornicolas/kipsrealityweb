...
export const utilityService = {
  ...
  async calculateAllocations(utilityBillId: string) {
    ...
    if (utilityBill.status === UtilityBillStatus.POSTED)
    ...
    switch (utilityBill.splitMethod) {
      ...
      case UtilitySplitMethod.SQ_FOOTAGE: {
        const totalSqFt = activeLeases.reduce(
          (sum, l) => sum.plus(new Decimal(l.unit.squareFootage || 0)),
          new Decimal(0),
        );
    ...
    if (!diff.isZero() && allocations.length > 0) {
      allocations[allocations.length - 1].amount =
        allocations[allocations.length - 1].amount.plus(diff);
    }
  ...
  async postAllocations(utilityBillId: string, allocationsInput: any[]) {
    return await prisma.$transaction(
      async (tx) => {
        ...
        const masterGLEntry = await journalService.postJournalEntry({
        ...
        const recoveryResultsWithJournals = await Promise.all(
          recoveryResults.map(async (res, index) => {
            ...
            const recoveryGLEntry = await journalService.postJournalEntry({
              organizationId: utilityBill.property.organizationId!,
              date: new Date(),
